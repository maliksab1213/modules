...
module.exports.run = async ({ api, event,args }) => {  {
  ...
  if (!text2) return api.sendMessage('Please enter the correct format [text1 | text2 | text3]!', event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://api.memegen.link/images/drowning/${text1}/${text2}/${text3}.png`)).pipe(fs.createWriteStream(__dirname+'/cache/poh.png')).on('close',() => callback());     
}}