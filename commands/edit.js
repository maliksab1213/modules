...
module.exports.run = async ({ api, event, args }) => {
  try {
    ...
    if (memeType === "list") {
      ...
      api.sendMessage(`📜 Available memes:\n\n${templateList}`, event.threadID, event.messageID);
      return;
    }

    if (!memeTemplates[memeType]) {
      api.sendMessage("Invalid meme type. Use 'edit list' to see available memes.", event.threadID, event.messageID);
      return;
    }
