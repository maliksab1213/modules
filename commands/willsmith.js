...
module.exports.run = async ({ api, event,args }) => {  {
    ...
    const request = global.nodemodule["request"];
	 ...
	 let text = args.join(" ")
  if (!text) return api.sendMessage('الرجاء إدخال التنسيق الصحيح [النص1 | النص 2]!', event.threadID, event.messageID);
  const length_0 = parseInt(text.length)
  const text1 = text.substr(0, text.indexOf(' | ')); 
  if (!text1) return api.sendMessage('الرجاء إدخال التنسيق الصحيح [النص1 | النص 2]!', event.threadID, event.messageID);
  const length = parseInt(text1.length)
  const text2 = text.split(" | ").pop()
  if (!text2) return api.sendMessage('الرجاء إدخال التنسيق الصحيح [النص1 | النص 2]!', event.threadID, event.messageID);
